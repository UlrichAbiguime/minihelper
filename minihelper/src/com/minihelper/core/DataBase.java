...
public class DataBase {
	 ...
	 * 什么是SQLiteOpenHelper ？
	 * 这个类主要生成一个数据库，并对数据库的版本进行管理。当在程序当中调用这个类的方法getWritableDatabase()
	 * 或者getReadableDatabase()方法的时候，如果当时没有数据，那么Android系统就会自动生成一个数据库。
	...
	public void insertItem() {
		...
		// 那么mOpenHelper辅助类负责建立这个数据库。如果数据库已经建立，那么直接返回一个可写的数据库。
		SQLiteDatabase db = mOpenHelper.getWritableDatabase();
		String sql1 = "insert into " + TABLE_NAME + " (" + TITLE + ", " + BODY + ") values('barfoo', 'android的发展真是迅速啊');";
		String sql2 = "insert into " + TABLE_NAME + " (" + TITLE + ", " + BODY + ") values('barfoo2', 'android的发展真是迅速啊');";
		try {
			// Log.i（）会将参数内容打印到日志当中，并且打印级别是Info级别
			// Android支持5种打印级别，分别是Verbose、Debug、Info、Warning、Error，当然我们在程序当中一般用到的是Info级别
			Log.i("barfoo:sql1=", sql1);
			Log.i("barfoo:sql2=", sql2);
			db.execSQL(sql1);
			db.execSQL(sql2);
			// setTitle("插入两条数据成功");
		} catch (SQLException e) {
	...
	public void deleteItem() {
		...
		try {
			...
			db.delete(TABLE_NAME, " title = 'barfoo'", null);
			// setTitle("删除title为barfoo的一条记录");
		} catch (SQLException e) {
			// setTitle("删除数据错误");
		}
		db.close();
	...
	/*
	 * 在屏幕的title区域显示当前数据表当中的数据的条数。 Cursor cur = db.query(TABLE_NAME, col, null,
	 * null, null, null, null)语句将查询到的数据放到一个Cursor 当中。
	 * 这个Cursor里边封装了这个数据表TABLE_NAME当中的所有条列。 query()方法相当的有用，在这里我们简单地讲一下。
	...
	public void showItems() {
		...
		cur.moveToFirst();
		while (!cur.isAfterLast() && (cur.getString(1) != null)) {
			Log.i("SqlLite", cur.getString(0));
			Log.i("SqlLite", cur.getString(1));
			cur.moveToNext();
		}
		cur.close();
		db.close();
